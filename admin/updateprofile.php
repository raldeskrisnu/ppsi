...
if ($result = mysqli_query($conn, $cek)) {
    ...
    if(mysqli_num_rows($result) > 0){
        $sql = "UPDATE user SET email='$email', name='$nama' , password = '" . md5($password) . "' WHERE email = '$iduser'";

        if (mysqli_query($conn, $sql)) {

            echo "<script>alert('Update data berhasil anda akan diarahkan logout terlebih dahulu !')
